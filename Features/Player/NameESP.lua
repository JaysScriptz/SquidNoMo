...
local Enabled = false
local Color = Color3.fromRGB(245, 245, 255)
...
local function create(player, character)
    ...
    gui.Name = "SquidNoMo_NameESP"
    gui.Adornee = head
    gui.Size = UDim2.fromOffset(190, 24)
...
function NameESP:IsEnabled() return Enabled end
function NameESP:GetState() return Enabled and "on" or "off" end
function NameESP:SetColor(color)
    if typeof(color) == "Color3" then
        Color = color
        if Enabled then refresh() end
    end
end
function NameESP:GetColor() return Color end
